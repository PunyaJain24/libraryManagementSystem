...
const RBook = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            const response = await axios.post(`${URL_PATH}/transactions/returnBook`, userData, {
                headers: {
            ...
            if(response.status === 201){
                setSuccess("Successfully returned the book");
                setError(null);
            }
        } catch (error) {
            if (error.response?.status === 500) {
                setSuccess("You already returned the book.");
                setError(null);
            } else {
        ...
        <div className="h-screen flex justify-center items-center bg-gray-900">
            <div className="bg-gray-800 p-8 rounded-lg shadow-lg grid grid-cols-2 gap-10 w-full max-w-5xl">
                ...
                <div className="text-white">
                    <h1 className="text-2xl font-bold mb-6 text-center">Return Book</h1>
